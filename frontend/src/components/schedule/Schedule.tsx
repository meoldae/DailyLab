...
const Schedule = () => {
    ...
    useLayoutEffect(() => {
        ...
        dataList.map((item) => {
            elementList.push(<ScheduleItem activeStatus={selectedDate == item.selectedDate} colorCode={item.colorCode} dateText={item.selectedDate} clickStatus={( differDate(new Date(item.selectedDate), joinDate) > -1 && (item.status !== "X" || differDate(new Date(item.selectedDate), new Date()) > -1))} clickEvent={setSelectedDate} />);
        });
