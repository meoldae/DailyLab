    ...
    return (
        <div className="child-[div:not(:last-child)]:mb-4">
            {props.type != "prev" ? contents.map((item, index) => {
                if(item.updateStatus) return <TodoHandleItem mode="update" key={index} info={item} updateItem={props.updateItem} categoryList={props.categoryList} changeTodoUpdateMode={props.changeItemUpdateMode} />
